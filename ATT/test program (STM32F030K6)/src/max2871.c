...
unsigned int read_MAX2871(void){
    ...
    //MAX2871_reg.R2 &= 0xE3FFFFFF; // pocistimo MUX 2-0
    //MAX2871_reg.R5 &= 0xFFFBFFFF; // pocistimo MUX 3
    //MAX2871_reg.R2 |= 0x10000000; // MUX 2 - true (MUX 1100)
    //MAX2871_reg.R5 |= 0x00040000; // MUX 3 - true
    ...
    MAX2871_reg.R6 = data_out; // spravimo register v struct
    return data_out;
}
...
int MAX2871_set_frequency(int frequency){
    ...
    MAX2871_reg.R0 = REG0;
    MAX2871_reg.R4 &= 0b11111111100011111111111111111111; //resetiramo DIVA bite
    MAX2871_reg.R4 |= DIV<<20; // Dodamo DIV
